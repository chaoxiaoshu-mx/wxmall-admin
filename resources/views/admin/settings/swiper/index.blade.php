...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"> <a class="btn btn-primary radius" onclick="swiper_add('添加轮播图','/admin/home/swiper/create')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加轮播图</a></span> <span class="r">共有数据：<strong>54</strong> 条</span> </div>
	<div class="mt-20">
			...
			<tbody>
				@foreach($swipers as $swiper)
					<tr class="text-c">
						<td>{{$swiper->id}}</td>
						<td>
							<img width="210" class="swiper-thumb" src="{{$swiper->image_src}}">
						</td>
					    <td>{{$swiper->navigator_url}}</td>
					    <td>{{$swiper->open_type}}</td>
					    <td>{{$swiper->goods_id}}</td>
					    <td>{{$swiper->updated_at}}</td>
						<td class="td-manage"><a style="text-decoration:none" onClick="swiper_stop(this,'10001')" href="javascript:;" title="下架"><i class="Hui-iconfont">&#xe6de;</i></a> <a style="text-decoration:none" class="ml-5" onClick="swiper_edit('修改轮播图','/admin/home/swiper/{{$swiper->id}}/edit')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> <a style="text-decoration:none" class="ml-5" onClick="swiper_del(this,'{{$swiper->id}}')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				    </tr>
				@endforeach
			</tbody>
		</table>
...
/*轮播图-查看*/
function swiper_show(title,url,id){
	var index = layer.open({
		type: 2,
		title: title,
		content: url
...
/*轮播图-编辑*/
function swiper_edit(title,url){
	var index = layer.open({
		type: 2,
		title: title,
		content: url
